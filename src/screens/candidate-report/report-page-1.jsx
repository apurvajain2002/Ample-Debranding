...
const ReportPage1 = () => {
  ...
  const { selectedCandidateId, selectedJobId, selectedRoundId } =
    location.state || {};
  ...
  // Helper function to get score color based on AI score
  const getScoreColor = (aiScore) => {
  ...
  // Helper function to format AI score display
  const formatAiScore = (aiAnswer) => {
    if (!aiAnswer) return "0.0";
    if (aiAnswer.aiScore === "null") return "-";
    return aiAnswer.aiScore;
  };
  ...
  const generatePDF = async () => {
    ...
    }, 5000); // Delay the screenshot by 5000ms (5 seconds)
  };
  ...
  useEffect(() => {
    if (id) {
      dispatch(fetchDomainScores({ selectedJobId, selectedCandidateId }))
        .then((domainScoreData) => {
          setDomainScores(
            domainScoreData?.payload?.roundSpecificHashMap["Recruiter Round"][
              selectedCandidateId
            ]
          );
        })
    ...
    if (id) {
      dispatch(
        ...
        .then((audioRecordings) => {
          setAudioRecordings(
            audioRecordings?.payload?.candidateFiltrations?.find(
  ...
  useEffect(() => {
    if (id) {
      dispatch(fetchVideoScores({ selectedJobId }))
        .then((videoScoreData) => {
          setVideoScores(
            videoScoreData?.payload?.roundSpecificHashMap["Recruiter Round"][id]
          );
          setR2DomainSkills(
            videoScoreData?.payload?.roundSpecificHashMap[
  ...
  useEffect(() => {
    if (id) {
      dispatch(fetchAudioScores({ selectedJobId }))
        .then((audioScoreData) => {
          setAudioScores(
            audioScoreData.payload.roundSpecificDataDTO["Recruiter Round"].find(
              (item) => item.userId === id
            )
          );
        })
  ...
  const avgSoftSkillScore =
    r2DomainSkills?.filter((scoreData) => scoreData.skillType === "Soft")
      .length > 0
      ? (
          r2DomainSkills
            .filter((scoreData) => scoreData.skillType === "Soft")
  ...
  const avgR2DomainScore =
    r2DomainSkills?.filter((scoreData) => scoreData.skillType === "Domain")
      .length > 0
      ? (
          r2DomainSkills
            .filter((scoreData) => scoreData.skillType === "Domain")
            .reduce((acc, scoreData) => acc + scoreData.score, 0) /
          r2DomainSkills.filter((scoreData) => scoreData.skillType === "Domain")
            .length
        ).toFixed(1)
  ...
  const weightedAverageScore = (
    parseFloat(avgSoftSkillScore) * 0.5 +
    parseFloat(avgR2DomainScore) * 0.3 +
    parseFloat(avgR1DomainScore) * 0.2
  ).toFixed(1);

  const currentWorkEx = candidateInfo?.workExperience?.find(
    (experience) => experience.currentOrganization === true
  );
  ...
  return (
    <div className="rep-container" ref={page1Ref}>
      <div className="report-container report-1container">
        <header className="report-header">
          <div className="report-logo">
            <img src={rootColor.logoUrl} alt="abc" />
          </div>
        </header>
        <section
      ...
      <section
        ...
        // style={{ border: "2px solid #47143d",margin:"10px auto"}}
          ...
          <tbody>
            <tr>
              <td>
                ...
                {currentWorkEx?.designation}{" "}
              </td>
              <td>
                <span>Email ID </span> {candidateInfo?.primaryEmailId}
              </td>
              <td>
                <span>Current Organization</span>{" "}
                {currentWorkEx?.organizationName}
              </td>
              <td>
                <span>Name</span> {selectedCandidateInfo?.name}
              </td>
            </tr>
            <tr>
              <td>
                <span>Date of birth</span> {candidateInfo?.dateOfBirth}{" "}
              </td>
              <td>
                <span>Year of Experience</span> {candidateInfo?.totalExperience}
              </td>
              <td>
                <span>Mobile No</span> {candidateInfo?.mobileNumber1}
              </td>
              <td>
                <span>WhatsApp No</span> {candidateInfo?.whatsappNumber}
              </td>
            </tr>
            <tr>
              <td>
              ...
              <td>
                <span>Current CTC</span> {candidateInfo?.currentCTC}
              </td>
              <td>
                <span>Notice Period</span> {candidateInfo?.noticePeriod}
              </td>
              <td>
                <span>Age</span>{" "}
              </td>
      ...
      >
        ...
        <div className="Information-details-wr">
          <ul>
            {candidateInfo?.userAcademics?.map((academic, index) => (
              <li key={academic.userAcademicId}>
                ...
                <div className="ac-inf-right">
                  <h4>
                    {academic.degreeName} ({academic.universityName})
                  </h4>
           ...
           ${
             academic.endDate
               ? new Date(academic.endDate).toISOString().split("T")[0]
               : "Present"
           }`}
                  </p>
          ...
          <tbody>
            {candidateInfo?.workExperience?.map((experience, index) => (
              <tr key={experience.workExperienceId}>
                ...
                <td>
                  <span>Designation</span> {experience.designation}
                </td>
                <td>
                  <span>Duration</span>{" "}
              ...
              <div className="scorebox">
                ...
                <div className="scoregraph-wr scoregraph-2">
                  <div className="multigraph">
                    ...
                    >
                      {" "}
                      {r2DomainSkills?.filter(
                        (scoreData) => scoreData.skillType === "Soft"
                      ).length > 0
                        ? avgSoftSkillScore
                        : "0.0"}
              ...
              <div className="scorebox">
                ...
                <div className="scoregraph-wr scoregraph-2">
                  <div className="multigraph">
                    ...
                    >
                      {" "}
                      {avgR1DomainScore > 0 ? avgR1DomainScore : "0.0"}
  ...
  '0.0'
            ...
            <aside className="col xl4 l4 m4 s4 cspadding">
              <div className="scorebox" style={{ paddingBottom: 0 }}>
                <h2>Weighted Avg Score</h2>
                <h2 className="avg-goldent" style={{ fontSize: "32px" }}>
                  {weightedAverageScore}
                </h2>

                <ul className="svgscore-ul">
                  <li>
                    <span className="black-square"></span> R2 Soft Skills
                  </li>
                  <li>
                    <span className="green-square"></span> R2 Domain Skills
                  </li>
                  <li>
                    <span className="red-square"></span> R1 Domain Skills
                  </li>
                </ul>
                <h2
                  className="avgtricolor"
                  style={{ textAlign: "left", margin: "0 0 0 22px" }}
                >
                ...
                <div className="scoreboard-inn">
                  <table className="table table-scoreboard">
                    <tbody>
                      {r2DomainSkills
                        ?.filter(
                          (scoreData) => scoreData.skillType === "Domain"
                        )
                        .map((scoreData, index) => (
                          <tr key={index}>
                            ...
                            <td>
                              <div className="score-rope">
                                <div
                                  className={`scorerope-inn ${
                                    scoreData.averageScore
                                      ? `sc-${scoreData.averageScore * 10}`
                                      : ""
                                  }`}
      ...
      <section
        className="report-table-wr mt15 min-325"
        style={{ width: "96%", marginLeft: "2%", marginTop: "2%" }}
      >
        <header>Round 1 Interview Scores</header>
              ...
              <div className="scoregraph-wr scoregraph-2">
                <div className="multigraph">
                  <span
                    className="graph "
                    style={{
                      "--score": `${((item.averageScore ?? 0) * 180) / 100}deg`,
                    }}
                  >
                    {item.averageScore ?? 0}
                  </span>
          ...
          <div className="scoregraph-wr scoregraph-2">
            <div className="multigraph">
              <span
                ...
                style={{
                  "--score": `${
                    ((audioScores?.voicePitch ?? 0) * 180) / 100
                  }deg`,
                }}
              >
                {audioScores?.voicePitch ?? 0}
              </span>
              {/* <span className="circle" style={{
          ...
          <div className="scoregraph-wr scoregraph-2">
            <div className="multigraph">
              <span
                ...
                style={{
                  "--score": `${
                    ((audioScores?.speechFluency ?? 0) * 180) / 100
                  }deg`,
                }}
              >
                {audioScores?.speechFluency ?? 0}
              </span>
              {/* <span className="circle" style={{
          ...
          <div className="scoregraph-wr scoregraph-2">
            <div className="multigraph">
              <span
                ...
                style={{
                  "--score": `${
                    ((audioScores?.speakingSpeed ?? 0) * 180) / 100
                  }deg`,
                }}
              >
                {audioScores?.speakingSpeed ?? 0}
              </span>
              {/* <span className="circle" style={{
          ...
          <div className="scoregraph-wr scoregraph-2">
            <div className="multigraph">
              <span
                ...
                style={{
                  "--score": `${
                    ((audioScores?.pausePer15Sec ?? 0) * 180) / 100
                  }deg`,
                }}
              >
                {audioScores?.pausePer15Sec ?? 0}
              </span>
              {/* <span className="circle" style={{
          ...
          <div className="scoregraph-wr scoregraph-2">
            <div className="multigraph">
              <span
                ...
                style={{
                  "--score": `${
                    ((audioScores?.audioScore ?? 0) * 180) / 100
                  }deg`,
                }}
              >
                {audioScores?.audioScore ?? 0}
              </span>
              {/* <span className="circle" style={{
      ...
      <section
        ...
        style={{ width: "96%", marginLeft: "2%", marginTop: "2%" }}
      >
        <header>Hiring Manager Action</header>
        
        <div className="" style={{display:"flex", alignItems:"center", height:"50px",width:"100%",backgroundColor:"#f005"}}>
          <button className="report_button">
          <img src={icon.audioIcon} alt="Audio" /> 
          <p style={{fontSize:"12px",color:"#fff"}}>Add Voice Note</p>
          </button>
          <button className="report_button" style={{backgroundColor:"#fff"}}>
          <img src={icon.textIcon} alt="Text" /> 
          <p style={{fontSize:"12px",color:"#000"}}>Add Text Note</p>
          </button>
          <NormalInputField
      ...
      >
        ...
        <div className="table-body-q">
          {totalQuestionList.map((item, index) => {
            const aiAnswer = canRespList?.find(
              ...
              <div className="row valign-wrapper" key={index}>
                ...
                <aside className="col xl5 l5 m5 cspadding">
                  <div className="table-left-b">
                    ...
                    {/* Justified text inline style */}
                    {aiAnswer &&
                      aiAnswer.response &&
                      videoUrl !== aiAnswer.response && (
                        <a
                          className={`play-videoa ${
                            isGeneratingPDF ? "hide-in-pdf" : ""
                          }`}
                ...
                <aside className="col xl2 l2 m2 cspadding">
                  <div className="table-left-b">
                    <p
                      className="sc-p"
                      style={{ color: getScoreColor(aiAnswer?.aiScore) }}
                    >
                      {formatAiScore(aiAnswer)}
                    </p>
                  </div>
                </aside>
        ...
        <div className="audio-boxwr">
          {audioList && audioList.length > 0 ? (
            audioList.map((audio, index) => (
              <a
                key={audio.id}
                className={`play-videoa play-audio ${
                  currentAudioUrl === audio.response ? "playing" : ""
                }`}
                style={{ cursor: "pointer" }}
