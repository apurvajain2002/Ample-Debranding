...
import { setReportLink, setSelectedCandidateId } from "../../redux/slices/interview-responses-l1-dashboard-slice";
  ...
  useEffect(() => {
    if (id) {
      ...
      .then((domainScoreData) => {
        setDomainScores(domainScoreData?.payload?.roundSpecificHashMap["Recruiter Round"][selectedCandidateId]   ); 
      })
      .catch((error) => {
        console.error("Error fetching candidate info:", error);
      });
        ...
        <div className="row valign-wrapper" key={index}>
          ...
          <aside className="col xl5 l5 m5 cspadding">
            <div className="table-left-b">
              ...
              <p style={{ textAlign: 'justify' }}>{item.questionText}</p> {/* Justified text inline style */}
